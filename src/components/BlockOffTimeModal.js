...
const BlockOffTimeModal = ({ block, onBlock, onClose }) => {
  const [startTime, setStartTime] = useState('');
  const [endTime, setEndTime] = useState('');
  const [error, setError] = useState(null);
  const [isSubmitting, setIsSubmitting] = useState(false);

  // Parse the containing availability block's time range
  const parseBlockTime = (isoOrHHmm) => {
  ...
  const blockStartDT = block ? parseBlockTime(block.start) : null;
  const blockEndDT = block ? parseBlockTime(block.end) : null;
  ...
  useEffect(() => {
    if (blockStartDT && blockEndDT) {
      // Default to block start + 30min as a reasonable initial selection
      const defaultStart = blockStartDT;
      const defaultEnd = defaultStart.plus({ minutes: 30 });
      setStartTime(formatFor12h(defaultStart.hour, defaultStart.minute));
      setEndTime(formatFor12h(
        Math.min(defaultEnd.hour + (defaultEnd.minute > 0 ? 0 : 0), blockEndDT.hour),
        defaultEnd <= blockEndDT ? defaultEnd.minute : blockEndDT.minute
      ));
    }
  }, [block]);

  const generateTimeOptions = () => {
    if (!blockStartDT || !blockEndDT) return [];
    const options = [];
    let current = blockStartDT;
    while (current < blockEndDT) {
      const label = formatFor12h(current.hour, current.minute);
      options.push(<option key={label} value={label}>{label}</option>);
      current = current.plus({ minutes: 30 });
    }
    // Also include the block end time itself as an option for the end picker
    const endLabel = formatFor12h(blockEndDT.hour, blockEndDT.minute);
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const localDate = blockStartDT.toFormat('yyyy-MM-dd');
      await onBlock({
        date: localDate,
  ...
  const blockRangeLabel = blockStartDT && blockEndDT
    ? `${blockStartDT.toFormat('h:mm a')} - ${blockEndDT.toFormat('h:mm a')}`
    : '';
